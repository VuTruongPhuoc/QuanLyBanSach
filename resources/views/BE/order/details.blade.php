...
@section('content')
                                ...
                                <div class="panel-body">
                                            ...
                                            <tbody>
                                                ...
                                                @foreach ($order_detail as $order_details)
                                                    <tr data-expanded="true">
                                                        ...
                                                        <td>{{ number_format($order_details->quantity * $order_details->price) . ' ' . 'VND' }}
                                                        </td>
                                                        @php
                                                            $total += $order_details->quantity * $order_details->price;
                                                        @endphp
                                                    </tr>
                                                @endforeach
                                            </tbody>
                                    ...
                                    <div class="row" style="margin-bottom: 20px;">
                                        ...
                                        <div class="col-lg-6" style="display: flex; align-items: center;">
                                            ...
                                            <div class="col-lg-4">
                                                <form action="/admin/order/update/{{$orders->id}}" method="post" id="orderStatusForm1">
                                                    @csrf
                                                    <select onchange='this.form.submit()' name="order_status" class="form-control m-bot15"
                                                            @if ($orders->order_status == 3 || $orders->order_status == 4) disabled="disabled" @endif>
                                                          
                                                        <option value="{{ $orders->order_status }}">
                                                            {{ config('app.order_status.' . $orders->order_status) }}
                                                        </option>
    ...
    <script>
        function updateFormAction() {
            var form2 = document.getElementById('orderStatusForm2');
            var selectedValue = form2.querySelector('select').value;
    
            // Check if the selected value is 3
            if (selectedValue == 3) {
                form2.action = '/admin/update_subtotal';
            }
